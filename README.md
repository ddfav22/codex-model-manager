...
一个面向 Windows 的 Codex 渠道、API Key、模型目录与本地协议适配管理器。当前版本：**1.2.60**。
...
- 为仅提供 Chat Completions 的 Grok 渠道适配 Responses 事件、工具调用、工具结果与有限 Agent Loop 恢复；计划型中间消息按 `commentary` delta 渐进显示，不再等恢复请求结束后一次性出现。
