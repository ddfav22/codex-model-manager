...
一个面向 Windows 的 Codex 渠道、API Key、模型目录与本地协议适配管理器。当前版本：**1.2.55**。
...
- 为仅提供 Chat Completions 的 Grok 渠道适配 Responses 事件、工具调用、工具结果与有限 Agent Loop 恢复。
- 保留 Codex 的 `sessions`、Projects、Skills、Agents 和登录配置；导入、导出和删除操作包含隔离与失败回滚。
...
- `electron/protocol/`：模型别名、工具连续性、协议转换等可复用模块。
- `electron/runtime/`：窗口、静态页面、IPC、安全边界与生命周期。
- `electron/features/`：更新、导入、用户错误等独立功能。
