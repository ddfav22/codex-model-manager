...
const modelManagerCore = read('modelManagerCore.ts')
const channelRow = read(path.join('components', 'ChannelRow.tsx'))
...
assert.match(modelManagerCore, /provider\.keySource === 'newapi'/)
assert.match(modelManager, /NewAPI 渠道按当前 Key 的 \/v1\/models 结果显示 ChatGPT\/OpenAI 模型/)
assert.match(channelRow, /const directNewApi = provider\.keySource === 'newapi'/)
assert.match(channelRow, /!directNewApi && Boolean\(provider\.baseUrl\)/)
